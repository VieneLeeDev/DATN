        ...
        let _return: readonly any[] = self.itemsSorted.filter(
          (room: any) =>
            ...
            (!root.filter.filter_city ||
              room.hotel?.city === root.filter.filter_city) && //filter base on the city of the room
  ...
  .actions((self: any) => {
    return {
      ...
      itemSelected() {
        ...
        return values(root.room.items).find((room: any) => room.id === root.filter.filter_selected);
      },
    };
  });
