...
import { BookingStore, bookingStore } from "@/stores/booking/BookingStore";
...
const DetailRoom = ({ params }: { params: { id: string } }) => {
  ...
  useEffect(() =>{
    ...
    dataInSearch.checkIn !== "" ? setDataInBill({checkIn:String(dateCheckIn),checkOut:String(dateCheckOut)}):setDataInBill({checkIn:String(dayjs().format("MM-DD-YYYY")),checkOut:String(dayjs().format("MM-DD-YYYY"))})
 ...
 const handleOrder = () => {
  if(detailRoom){
    ...
    const dataBooking = {id:idBooking,id_room:detailRoom?.id,date_checkIn:dataInBill.checkIn,date_checkOut:dataInBill.checkOut,total_price:detailRoom?.price * count,status:"Confirmed"}
