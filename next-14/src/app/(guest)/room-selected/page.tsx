...
const DetailRoom = observer(({params}: { params: { id: string } }) => {
    ...
    useEffect(() => {
        ...
        dataInSearch.checkIn !== "" ? setDataInBill({
            checkIn: '',
            checkOut: ''
        }) : setDataInBill({
    ...
    const handleOrder = () => {
        ...
        //         status: "Confirmed"
        //     }
        //     router.push(`/received/${idBooking}`)
        // }
    }
        ...
        <div className="container flex flex-col h-full justify-center items-center lg:py-[30px]">
            ...
            <div className="container relative h-[400px] bg-slate-100 flex justify-center items-center">
                <img
                    ...
                    alt="pic"
                    src={roomSelected ? `${roomSelected?.image_url}` : "https://fl-1.cdn.flockler.com/embed/no-image.svg"}/>
            </div>
            <hr className="container my-10"/>
            <div className="container flex flex-col md:flex-row ">
                <div className="flex-[2]">
                    <p>
                        {roomSelected.hotel?.description}
                    </p>
                    {/**Detail */}
                    <div
                        className="flex-col lg:flex-row flex container min-h-[300px] border-[1px] p-5 my-10 lg:justify-between">
                        <h2 className="flex-[1] text-center text-2xl font-bold">Details</h2>
                        <div
                            className="flex-[3] h-1/3 border-t-[1px] lg:h-full lg:px-10 lg:border-l-[1px] lg:border-t-0">
                            <ul>
                                ...
                                <li className="flex my-5 break-words">
                                    ...
                                    <span className="w-3/4">{roomSelected?.city}</span>
                                </li>
                            </ul>
                        </div>
                    </div>
                  ...
                  <strong>€{roomSelected?.price}</strong>
                        ...
                        <div className="h-3/4">
                            <div className="flex flex-col my-5 ">
                                ...
                                {<DatePicker disabledDate={ValidateCheckIn} defaultValue={defaultValueCheckin}
                                             size="large" format="MM-DD-YYYY"
                                             onChange={(obj, value) => (onChangeDateCheckIn(value))}/>}
                            </div>
                            <div className="flex flex-col my-5 ">
                                <p className="text-sm font-semibold">Check-out Date*</p>
                                {<DatePicker disabledDate={ValidateCheckOut} defaultValue={defaultValueCheckOut}
                                             size="large" format="MM-DD-YYYY"
                ...
                <span className="text-2xl">
                  <strong>€{`${dataInBill.checkIn !== "" ? roomSelected && roomSelected?.price * count : roomSelected?.price}`}</strong>
                </span>{" "}
                                for {dataInBill.checkIn !== "" ? count : 1} days
                            </p>
                            <button className="w-[150px] h-[50px] bg-slate-200" onClick={handleOrder}>Order</button>
                        </div>
                    </div>
                </div>
            </div>
